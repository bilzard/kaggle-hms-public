...
class HmsModelLeakageCheck(nn.Module):
    """
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        x = self.feature_processor(
            dict(spec=x, spec_mask=output["spec_mask"], weight=batch[self.weight_key])
        )
        x = self.head(x)
...
def check_model(
    ...
    x = model.feature_processor(dict(spec=x, spec_mask=spec_mask, weight=weight))
