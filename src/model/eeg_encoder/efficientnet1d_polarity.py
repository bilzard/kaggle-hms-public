...
class EfficientNet1dPolarity(nn.Module):
    """
    極性の反転の特徴を入力する。
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x_all = torch.cat([x, x_p0, x_p1], dim=0)
        x_all = self.polarity_encode(x_all)  # (3 b) c ch t/k
        x_src = x_all[:b]
        x_p0 = x_all[b : 2 * b]
        x_p1 = x_all[2 * b :]
        ...
        x = self.stem_conv(x)
        x_feat = torch.cat([x, sim_p0, sim_p1], dim=1)
        x = self.feat_mapper(x_feat)
...
if __name__ == "__main__":
    ...
    model = EfficientNet1dPolarity(in_channels)
