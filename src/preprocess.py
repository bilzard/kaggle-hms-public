...
def do_process_cqf(
    ...
    """
    Calculate Channel Quality Factor(CQF)
    """
    ...
    for p in EEG_PROBES:
        eeg_df = eeg_df.with_columns(
            pl.col(f"{cqf_metric}-{p}").lt(distance_threshold).alias(f"mask-{p}"),
            pl.lit(1.0)
            .truediv(
                pl.col(f"{cqf_metric}-{p}").clip(0).truediv(distance_threshold).add(1.0)
            )
            .alias(f"CQF-{p}"),
        )
...
def process_label(
    ...
    if only_use_sp_center and not use_cache:
        # c.f. https://www.kaggle.com/competitions/hms-harmful-brain-activity-classification/discussion/492471#2746447
        dfs = []
        for eeg_id, eeg_df in tqdm(
            metadata.group_by("eeg_id", maintain_order=True),
            total=metadata["eeg_id"].n_unique(),
        ):
            eeg_df = eeg_df.with_columns(
                pl.concat_str(
                    [eeg_df[f"{label}_vote"].cast(pl.Utf8) for label in LABELS],
                    separator="_",
                ).alias("vote_id")
            )
            rows = []
            for vote_id, vote_df in eeg_df.group_by("vote_id", maintain_order=True):
                offsets = (
                    vote_df["spectrogram_label_offset_seconds"].to_numpy().astype(int)
                )
                x = np.zeros(offsets.max() + 600)
                for o in offsets:
                    x[o : o + 600] += 1
                best_idx = np.argmax([x[o : o + 600].sum() for o in offsets])
                rows += [vote_df[best_idx : best_idx + 1]]
            best_vote_df = pl.concat(rows)

            dfs.append(best_vote_df)

        metadata = pl.concat(dfs)
    ...
    votes = metadata.select(f"{label}_vote" for label in LABELS).to_numpy()
    max_vote_idx = np.argmax(votes, axis=1)
    expert_consensus = np.array(LABELS)[max_vote_idx]
    metadata = metadata.with_columns(
        pl.Series(expert_consensus).alias("expert_consensus"),
    )
