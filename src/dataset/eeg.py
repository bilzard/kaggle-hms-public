...
class PerLabelDataset(HmsBaseDataset):
    def __init__(
        self,
        metadata: pl.DataFrame,
        id2eeg: dict[int, np.ndarray],
    ...
    def __getitem__(self, idx):
        ...
        if self.with_label:
            label = np.array(
                [
                    [row[f"{label}{postfix}"] for label in LABELS]
                    for postfix in self.label_postfix
                ],
                dtype=np.float32,
            )
            weight = np.array(
                [row[key] for key in self.weight_key],
                dtype=np.float32,
            )
            data |= dict(label_id=label_id, label=label, weight=weight)
